(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('hedwigApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-hedwigApp-error');
                var entityKey = httpResponse.headers('X-hedwigApp-params');
                if (errorHeader) {
