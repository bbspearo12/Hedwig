...
public class ASUPAlertDataResource {
	...
	public List<ASUPAlertData> getAllASUPAlertDataByASUPID(@PathVariable String id) {
		...
		return aSUPAlertDataService.findAllByASUP(id);
	}

	/**
	 * GET  /a-sup-alert-data/asup/{id}/{file} : get all the aSUPAlertData for a given asup id.
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of aSUPAlertData in body
	 */
	// TODO implement this in the service layer
	@RequestMapping(value = "/a-sup-alert-data/asupfile/{id}",
			method = RequestMethod.GET,
			produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<ASUPAlertData> getAllASUPAlertDataByASUPIDforFile(@PathVariable("id") String id, @RequestParam("filename") String filename) {
		log.debug("REST request to get ASUPAlertData for ASUP id: {} and file: {}", id, filename);
		ASUPAlertData asupMatched = null;
		List<ASUPAlertData> all = aSUPAlertDataService.findAllByASUP(id);
		for (ASUPAlertData alert : all) {
			if (alert.getAsup_alert_file_name().equalsIgnoreCase(filename)) {
				asupMatched = alert;
			}
		}
		log.debug("Returning data: {} for id: {}", asupMatched, id);
		return Optional.ofNullable(asupMatched)
